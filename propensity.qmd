...
# Modeling for Observational Treatment Comparisons {#sec-os}

**Overview**

* Challenges of observational treatment studies
   + no blinding
   + often difficult to define time zero
   + confounding by indication
   + no unique way to adjust for measured confounders
   + unmeasured confounders
* Covariate adjustment
   + powerful
   + must use penalization or Bayesian shrinkage priors if # covariates large with respect to effective sample size (ESS) for $Y$
* Propensity scores (PS)
   + PS = estimated exposure tendency given baseline covariates
   + a data reduction procedure useful when ESS for exposure $>>$ ESS for $Y$
   + is a _outcome heterogeneity-ignoring confounder funnel_ that reduces a large set of confounders to 1 variable
   + assumes confounders are measured
   + $\rightarrow$ not a _causal inference_ tool per se
   + inefficient if weight or match on PS
   + most matching algorithms discard valuable observations or are dataset order-dependent
   + lulls analysts into ignoring outcome heterogeneity
   + $\rightarrow$ underestimates exposure effect in nonlinear models
   + grossly inflated $\sigma$ in linear models when $R^2$ not tiny
   + hides interactions with exposure
   + need to take uncertainties in PS coefficients into account in analysis
* Best practice
   + use penalization with full covariate adjustment, or use PS
   + model PS: exposure ~ additive model in all potential confounders $C$
      - spline all continuous $C$ if ESS in exposure allows
      - no need to check for balance in $C$ conditional on PS
   + model $Y$ ~ spline(logit(PS)) + $X$ + exposure
      - $X$ = pre-specified strong prognostic factors
---
...
* Need to take PS estimation uncertainty into account when matching or weighting
...
1. If the number of potential confounders is large, consider penalized estimation or Bayesian shrinkage priors
1. Model the probability of receiving treatment using a flexible
  ...
  there are treatments, less one, and all of the logit propensity
  scores will need to be adjusted for in what follows.
1. What is balanced depends on how confounders are modeled
   - linear: means are equal
   - quadratic: means and variances are equal
   - cubic: means, variances, skewness are equal
   - spline with enough knots: balances entire distribution
1. With flexible modeling, no need to check for balance in confounders conditional on PS
...
1. If there is a non-overlap region of the two distributions, and
  ...
  understood.  Logistic regression will work even better.
...
[Assumptions in the use of propensity scores](https://stats.stackexchange.com/questions/656403) including a discussion of whether propensity analysis is "causal".
