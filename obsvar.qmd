...
# Analysis of Observer Variability and Measurement Agreement {#sec-obsvar}

[This chapter was written by FE Harrell, 1987]{.aside}
...
Hoeffding @hoe48cla^[The Wilcoxon test and the $c$-index are other examples of $U$ statistics.]. For definiteness, an analysis for 3 observers and 2 readings per observer will be
shown. When designing such a study, the researcher should remember
that the number of experimental units is usually the critical factor
...
|8-5| + |6-7|)/3 = \frac{2+3+1}{3} = 2$.

The mean inter-observer difference is
...
+ |7-5| + |7-6| + |7-7| + |8-6| + |8-7| + |5-6| + |5-7|)/8 =$ $(1+2+1+0+2+1+1+2)/8 = \frac{10}{8} = 1.25$.
...
developed much more general purpose software for this in `R`.  
See [github.com/harrelfe/rscripts](https://github.com/harrelfe/rscripts).
...
$p_{A} \pm 1.96 \sqrt{p_{A} (1 - p_{A})/n}$~^[A more accurate confidence interval can be obtained using Wilson's method as provided by the `R` `Hmisc` package `binconf` function.]
