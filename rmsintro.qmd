...
`r ipacue()` 
**Note**: To easily run all the following commands, open [fharrell.com/code/bbr.zip](http://fharrell.com/code/bbr.zip) and then open the file `8-rmsintro.r` contained in the `.zip` file using `RStudio`.  Commands listed in previous sections were not actually executed so they are marked with the `R` comment symbol
(#) and can be ignored.
...
```
Enhanced output to better explain which parameters are being tested is available if you are not using `html`:
```{r plainanova}
