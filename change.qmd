...
`r quoteit('Not only is change from baseline a very problematic response variable; the very notion of patient improvement as an outcome measure can even be misleading.  A patient who starts at the best level and who does not worsen should be considered a success.')`
...
#### Nearly Optimal Statistical Model {#sec-change-opmod}
...
* Proportional odds model starts with treatment odds ratio which is [convertible to a Wilcoxon statistic concordance probability](https://fharrell.com/post/powilcoxon)
...
### KCCQ Ceiling Effect {#sec-change-kccq}
...
See [this](https://fharrell.com/post/pop) for another KCCQ example where the slope of post vs. pre is far from 1.0, and an example using the cardiac biomarker BNP where post vs. pre is far from linear.
...
require(rms)
load('~/i/Analyses/SUPPORT/combined.sav')
combined <- subset(combined,
  select=c(id, death, d.time, hospdead, dzgroup, age, raceh, sex))
load('~/i/Analyses/SUPPORT/combphys.sav')
...
<img src="images/cho17effFig3.png" width="100%">
...
Classic paper on shrinkage: @efr77ste
