...
#### Calprotectin Bayesian Wilcoxon Test {#sec-nonpar-cal-bayes}

* Repeat with a Bayesian fit, using posterior mean regression coefficients to estimate covariate-specific medians
* A better but slightly more involved approach would be to compute covariate-specific medians for all posterior draws of $\hat{\beta}$.
...
### Bayesian Mixed Effects Model for Rank Difference Test

* A Bayesian mixed effects PO model also works well
...
### Accounting for Within-Subject Correlation With Cluster Sandwich Estimator
